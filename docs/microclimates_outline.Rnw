...
site.ws.lci<-round(fit.noisyws[grep("mu_b_urban_sp", rownames(fit.noisyws)),4], digits=2)
site.ws.uci<-round(fit.noisyws[grep("mu_b_urban_sp", rownames(fit.noisyws)),8], digits=2)
site.ws.sd <- (site.ws.lci + site.ws.lci)/2

method.ws <- round(fit.noisyws[grep("mu_b_method_sp", rownames(fit.noisyws)),1], digits=2)
method.ws.lci<-round(fit.noisyws[grep("mu_b_method_sp", rownames(fit.noisyws)),4], digits=2)
method.ws.uci<-round(fit.noisyws[grep("mu_b_method_sp", rownames(fit.noisyws)),8], digits=2)
method.ws.sd <- (method.ws.lci + method.ws.lci)/2

um.ws <- round(fit.noisyws[grep("mu_b_um_sp", rownames(fit.noisyws)),1], digits=2)
um.ws.lci<-round(fit.noisyws[grep("mu_b_um_sp", rownames(fit.noisyws)),4], digits=2)
um.ws.uci<-round(fit.noisyws[grep("mu_b_um_sp", rownames(fit.noisyws)),8], digits=2)
um.ws.sd <- (um.ws.lci + um.ws.lci)/2
...
site.hobo.lci<-round(fit.noisyws[grep("mu_b_urban_sp", rownames(fit.noisyws)),4], digits=2)
site.hobo.uci<-round(fit.noisyws[grep("mu_b_urban_sp", rownames(fit.noisyws)),8], digits=2)
site.hobo.sd <- (site.hobo.lci + site.hobo.lci)/2

method.hobo <- round(fit.noisyhobo[grep("mu_b_method_sp", rownames(fit.noisyhobo)),1], digits=2)
method.hobo.lci<-round(fit.noisyhobo[grep("mu_b_method_sp", rownames(fit.noisyhobo)),4], digits=2)
method.hobo.uci<-round(fit.noisyhobo[grep("mu_b_method_sp", rownames(fit.noisyhobo)),8], digits=2)
method.hobo.sd <- (method.hobo.lci + method.hobo.lci)/2

um.hobo <- round(fit.noisyhobo[grep("mu_b_um_sp", rownames(fit.noisyhobo)),1], digits=2)
um.hobo.lci<-round(fit.noisyhobo[grep("mu_b_um_sp", rownames(fit.noisyhobo)),4], digits=2)
um.hobo.uci<-round(fit.noisyhobo[grep("mu_b_um_sp", rownames(fit.noisyhobo)),8], digits=2)
um.hobo.sd <- (um.hobo.lci + um.hobo.lci)/2
...
site.micros.lci<-round(fit.micros[grep("mu_b_urban_sp", rownames(fit.micros)),4], digits=2)
site.micros.uci<-round(fit.micros[grep("mu_b_urban_sp", rownames(fit.micros)),8], digits=2)
site.micros.sd <- (site.micros.lci + site.micros.lci)/2

method.micros <- round(fit.micros[grep("mu_b_method_sp", rownames(fit.micros)),1], digits=2)
method.micros.lci<-round(fit.micros[grep("mu_b_method_sp", rownames(fit.micros)),4], digits=2)
method.micros.uci<-round(fit.micros[grep("mu_b_method_sp", rownames(fit.micros)),8], digits=2)
method.micros.sd <- (method.micros.lci + method.micros.lci)/2

um.micros <- round(fit.micros[grep("mu_b_um_sp", rownames(fit.micros)),1], digits=2)
um.micros.lci<-round(fit.micros[grep("mu_b_um_sp", rownames(fit.micros)),4], digits=2)
um.micros.uci<-round(fit.micros[grep("mu_b_um_sp", rownames(fit.micros)),8], digits=2)
um.micros.sd <- (um.micros.lci + um.micros.lci)/2
...
site.urban.lci<-round(fit.urban[grep("mu_b_urban_sp", rownames(fit.urban)),4], digits=2)
site.urban.uci<-round(fit.urban[grep("mu_b_urban_sp", rownames(fit.urban)),8], digits=2)
site.urban.sd <- (site.urban.lci + site.urban.lci)/2

method.urban <- round(fit.urban[grep("mu_b_method_sp", rownames(fit.urban)),1], digits=2)
method.urban.lci<-round(fit.urban[grep("mu_b_method_sp", rownames(fit.urban)),4], digits=2)
method.urban.uci<-round(fit.urban[grep("mu_b_method_sp", rownames(fit.urban)),8], digits=2)
method.urban.sd <- (method.urban.lci + method.urban.lci)/2

um.urban <- round(fit.urban[grep("mu_b_um_sp", rownames(fit.urban)),1], digits=2)
um.urban.lci<-round(fit.urban[grep("mu_b_um_sp", rownames(fit.urban)),4], digits=2)
um.urban.uci<-round(fit.urban[grep("mu_b_um_sp", rownames(fit.urban)),8], digits=2)
um.urban.sd <- (um.urban.lci + um.urban.lci)/2
...
site.urbmethod.lci<-round(fit.urbmethod[grep("mu_b_urban_sp", rownames(fit.urbmethod)),4], digits=2)
site.urbmethod.uci<-round(fit.urbmethod[grep("mu_b_urban_sp", rownames(fit.urbmethod)),8], digits=2)
site.urbmethod.sd <- (site.urbmethod.lci + site.urbmethod.lci)/2

method.urbmethod <- round(fit.urbmethod[grep("mu_b_method_sp", rownames(fit.urbmethod)),1], digits=2)
method.urbmethod.lci<-round(fit.urbmethod[grep("mu_b_method_sp", rownames(fit.urbmethod)),4], digits=2)
method.urbmethod.uci<-round(fit.urbmethod[grep("mu_b_method_sp", rownames(fit.urbmethod)),8], digits=2)
method.urbmethod.sd <- (method.urbmethod.lci + method.urbmethod.lci)/2

um.urbmethod <- round(fit.urbmethod[grep("mu_b_um_sp", rownames(fit.urbmethod)),1], digits=2)
um.urbmethod.lci<-round(fit.urbmethod[grep("mu_b_um_sp", rownames(fit.urbmethod)),4], digits=2)
um.urbmethod.uci<-round(fit.urbmethod[grep("mu_b_um_sp", rownames(fit.urbmethod)),8], digits=2)
um.urbmethod.sd <- (um.urbmethod.lci + um.urbmethod.lci)/2
...
site.real.lci<-round(fit.real[grep("mu_b_urban_sp", rownames(fit.real)),4], digits=2)
site.real.uci<-round(fit.real[grep("mu_b_urban_sp", rownames(fit.real)),8], digits=2)
site.real.sd <- (site.real.lci + site.real.lci)/2

method.real <- round(fit.real[grep("mu_b_method_sp", rownames(fit.real)),1], digits=2)
method.real.lci<-round(fit.real[grep("mu_b_method_sp", rownames(fit.real)),4], digits=2)
method.real.uci<-round(fit.real[grep("mu_b_method_sp", rownames(fit.real)),8], digits=2)
method.real.sd <- (method.real.lci + method.real.lci)/2

um.real <- round(fit.real[grep("mu_b_um_sp", rownames(fit.real)),1], digits=2)
um.real.lci<-round(fit.real[grep("mu_b_um_sp", rownames(fit.real)),4], digits=2)
um.real.uci<-round(fit.real[grep("mu_b_um_sp", rownames(fit.real)),8], digits=2)
um.real.sd <- (um.real.lci + um.real.lci)/2
...
um.sigma.real <- round(fit.real[grep("sigma_b_up_sp", rownames(fit.real)),1], digits=2)
...
fit.provreal <- summary(urbmethod)$summary

site.provreal <- round(fit.provreal[grep("mu_b_urban_sp", rownames(fit.provreal)),1], digits=2)
site.provreal.lci<-round(fit.provreal[grep("mu_b_urban_sp", rownames(fit.provreal)),4], digits=2)
site.provreal.uci<-round(fit.provreal[grep("mu_b_urban_sp", rownames(fit.provreal)),8], digits=2)
site.provreal.sd <- (site.provreal.lci + site.provreal.lci)/2

method.provreal <- round(fit.provreal[grep("mu_b_method_sp", rownames(fit.provreal)),1], digits=2)
method.provreal.lci<-round(fit.provreal[grep("mu_b_method_sp", rownames(fit.provreal)),4], digits=2)
method.provreal.uci<-round(fit.provreal[grep("mu_b_method_sp", rownames(fit.provreal)),8], digits=2)
method.provreal.sd <- (method.provreal.lci + method.provreal.lci)/2

um.provreal <- round(fit.provreal[grep("mu_b_um_sp", rownames(fit.provreal)),1], digits=2)
um.provreal.lci<-round(fit.provreal[grep("mu_b_um_sp", rownames(fit.provreal)),4], digits=2)
um.provreal.uci<-round(fit.provreal[grep("mu_b_um_sp", rownames(fit.provreal)),8], digits=2)
um.provreal.sd <- (um.provreal.lci + um.provreal.lci)/2
...
um.sigma.provreal <- round(fit.provreal[grep("sigma_b_up_sp", rownames(fit.provreal)),1], digits=2)
...
site.prov.lci<-round(fit.prov[grep("mu_b_urban_sp", rownames(fit.prov)),4], digits=2)
site.prov.uci<-round(fit.prov[grep("mu_b_urban_sp", rownames(fit.prov)),8], digits=2)
site.prov.sd <- (site.prov.lci + site.prov.lci)/2

method.prov <- round(fit.prov[grep("mu_b_method_sp", rownames(fit.prov)),1], digits=2)
method.prov.lci<-round(fit.prov[grep("mu_b_method_sp", rownames(fit.prov)),4], digits=2)
method.prov.uci<-round(fit.prov[grep("mu_b_method_sp", rownames(fit.prov)),8], digits=2)
method.prov.sd <- (method.prov.lci + method.prov.lci)/2

um.prov <- round(fit.prov[grep("mu_b_um_sp", rownames(fit.prov)),1], digits=2)
um.prov.lci<-round(fit.prov[grep("mu_b_um_sp", rownames(fit.prov)),4], digits=2)
um.prov.uci<-round(fit.prov[grep("mu_b_um_sp", rownames(fit.prov)),8], digits=2)
um.prov.sd <- (um.prov.lci + um.prov.lci)/2
...
um.sigma.prov <- round(fit.prov[grep("sigma_b_up_sp", rownames(fit.prov)),1], digits=2)
...
\item Climate on a smaller scale may also be important to consider. 
  ...
  \item To facilitate scaling and minimize error due to these fine-scal climatic effects (i.e., microclimatic effects), researchers often deploy standalone weather loggers---such as HOBO sensors---which may provide higher resolution weather data \citep{Schwartz2013a,Whiteman2000}.
...
\begin{enumerate}
\item We find we can accurately recover a simple effect of urban sites requiring more GDD (Figure \ref{fig:urban} and Table \ref{tab:urban}).
\item Simulations that include microclimates at both sites show that the hobo loggers require more GDDs until budburst. 
  \item When simulating microclimatic effects across the sites, we include greater variation in temperature for the hobo logger data, which is being reflected by the negative slope of the method parameter (\Sexpr{method.micros} $\pm$ \Sexpr{method.micros.sd}). 
  \item Provenance effect simulations indicate more northern provenance locations require fewer GDDs until budburst which is being recovered in the provenance parameter (\Sexpr{site.prov} $\pm$ \Sexpr{site.prov.sd}; Figure \ref{fig:prov} and Table \ref{tab:prov}).
  \item When we manipulate the simulations to have noisy weather station data, noise is returned as the sigma for the method parameter (Figure \ref{fig:simsmus} \textbf{a)}) and the method parameter is slightly positive (\Sexpr{method.noisyws} $\pm$ \Sexpr{method.noisyws.sd}), indicating weather stations require more GDDs until budburst. 
  \item Though, when we manipulate the simulations to have noisy hobo logger data, the output is nearly identical (Figure \ref{fig:simsmus} \textbf{b)}) but the method parameter is slightly negative (\Sexpr{method.noisyhobo} $\pm$ \Sexpr{method.noisyhobo.sd}), now indicating hobo loggers require more GDDs until budburst.
  \end{enumerate}
  
  \subsection*{Simulations -- part 1 --  GDD accuracy}
\begin{enumerate}
\item The GDD model is most accurate for individuals that have high GDD thresholds and when base temperatures are higher (i.e., 10$^{\circ}$C; Figure \ref{fig:forecasts} \textbf{a)}). 
  \begin{enumerate}
  \item However, the GDD model becomes less accurate with warming, and accuracy decreases at a faster rate with the higher base temperature (i.e., 10$^{\circ}$C) than with the lower base temperature (i.e., 0$^{\circ}$C; Figure \ref{fig:forecasts} \textbf{b)}).
...
\begin{enumerate}
\item Individuals at the arboretum (i.e., more urban sites) require more GDDs to budburst (\Sexpr{site.real} $\pm$ \Sexpr{site.real.sd}).
  \begin{enumerate}
  \item There is high variation in GDDs between the two methods (\Sexpr{method.sigma.real}) though the slope is close to zero (method.real $\pm$ method.real.sd).
  \item There is a large interaction indicating weather station data at the arboretum records the fewest number of GDDs until budburst (\Sexpr{um.real} $\pm$ \Sexpr{um.real.sd}). 
  \item Using raw data, we see there is higher variation at the arboretum across the two methods and that the arboretum requires fewer GDDs until budburst than the Harvard Forest (i.e., more rural forest sites).
  \end{enumerate}

\item When testing for provenance effects, there was no overall effect of provenance latitude on GDDs until budburst and there was high variation (\Sexpr{site.provreal} $\pm$ (\Sexpr{site.provreal.sd})).
  \begin{enumerate}
  \item There was still little effect of method on GDDs until budburst and large variation (\Sexpr{method.provreal} $\pm$ \Sexpr{method.provreal.sd}).
  \item The interaction of provenance by method was close to zero (\Sexpr{um.provreal} $\pm$ \Sexpr{um.provreal.sd}) but the sigma was large (\Sexpr{um.sigma.provreal}).
\end{enumerate}
...
\item Our study assessed the effects of an urban arboretum versus a more rural forested site coupled with the effect of using weather station versus hobo logger temperature data on GDD until budburst.
  ...
  %\item This implies individuals may be receiving more over-winter chilling at the warmer site because optimum chilling accumulation temperatures generally occur above -2$^{\circ}$C and can accumulate at as high of temperatures as 10$^{\circ}C \citep{Baumgarten2021}.
  \item Our results additionally suggest there was a strong microclimate effect and it varied by site, with XX (but I think we can say here that both the Harvard Forest and Arboretum hobo loggers recorded lower temperatures overall than the weather stations but the Harvard Forest loggers recorded consistently lower temperatures)
  \item Provenance did not elucidate clear results so we suggest teasing out provenance effects, given that they may be so small in spring phenology \citep{Gauzere2017} and it is difficult with our paucity of latitudinal variation and sparce sample size for various provenances. 
  \end{enumerate}
  \end{enumerate}
