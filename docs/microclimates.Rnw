...
#### Now let's breakdown provenance data:
prov <- read.csv("output/provenanceinfo.csv")
provmin <- round(min(prov$provenance.lat, na.rm=TRUE), digits=2)
provmax <- round(max(prov$provenance.lat, na.rm=TRUE), digits=2)

prov$provenance.lat <- round(prov$provenance.lat, digits=1)
prov$provenance.lat <- ifelse(is.na(prov$provenance.lat), "none", prov$provenance.lat)
numprovs <- nrow(prov[(!prov$provenance.lat%in%c(42.3, "none")),])
...
We additionally examined the accuracy of GDD models using different base temperature thresholds in combination with warming through simulations. To evaluate the accuracy of GDD models, we used different base temperatures for GDD (i.e., we simulated cases where the species' base temperature was 0$^{\circ}$C versus 10$^{\circ}$C) with variation in sigma around mean temperatures (i.e., 0.1$^{\circ}$C and 1$^{\circ}$C, where higher sigma yield higher simulated variability in daily temperatures). We also tested GDD accuracy across various GDD threshold requirements with warming of 1$^{\circ}$C to 10$^{\circ}$C and using varying GDD threshold requirements for budburst without warming. Accuracy was evaluated as a ratio of observed GDD divided by the expected GDD, with perfect accuracy measured as 1. Values that deviate from 1 represent a percent change in inaccuracy (e.g., 1.1 is 10\% inaccurate; values are never less than 1 because observed GDD must always be equal to or greater than expected GDD in order for the threshold for budburst to be met).
...
Finally, we found no major effect of provenance latitude on GDDs until budburst though there was a slightly positive trend with higher provenance latitudes requiring more GDDs until budburst (\Sexpr{site.provreal} $\pm$ \Sexpr{site.provreal.sd} GDDs until budburst; Figure \ref{fig:prov} and Table \ref{tab:provreal}), but the variance around species was large (sigma of \Sexpr{site.sigma.provreal} GDDs until budburst). The effect of method on GDDs until budburst was close to zero (\Sexpr{method.provreal} $\pm$ \Sexpr{method.provreal.sd} GDDs until budburst). The interaction of provenance by method was also close to zero (\Sexpr{um.provreal} $\pm$ \Sexpr{um.provreal.sd} GDDs until budburst), but the variance across species was large (sigma of \Sexpr{um.sigma.provreal} GDDs until budburst).
...
% EMWDec31: New subsection, see what you think. (Also tweaked very end of discussion since I stole much of this text from there.)
...
% EMWDec31: Caption here could be one place to highlight what looks similar in our simulations?
\begin{figure}[H]
      \centering
...
% EMWDec31: check my caption edits, tried to explain sigma a litt;e
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
      \centering
      \includegraphics[width=16cm]{..//analyses/figures/gdd_interaction.pdf} % EMWDec31: check my caption edits
