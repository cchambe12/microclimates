...
\usepackage{natbib} %hyperref - attaches link to references
...
um.urbmethod.lci<-round(fit.urbmethod[grep("mu_b_um_fsp", rownames(fit.urbmethod)),5], digits=2)
...
%\section*{Highlights}
%\begin{itemize}
%\item Many phenological forecasts use growing degree day models (GDD).
%\item With a case study approach, we compare GDD estimates in two locations (urban, forest).
%\item We show common GDD models vary in accuracy across species, location, and climate. 
%\item Inaccuracy using GDD models may be especially high for early-active species. 
%\end{itemize}
...
%emw6Feb: I am a bit confused by the 'Crop Heat Unit' reference -- it feels pretty hacky. Why do we have it (some reviewer request)? Also, does the Crop Unit Heat model work for trees (or just herbs or what)? If we need to keep it, maybe adjust it from (but CHECK what I wrote seems accurate):
% Studies suggest other models---particularly the Crop Heat Unit \citep{Kumudini2014,Abendroth2019}---are more precise than the GDD model, though the GDD model is more widely accepted, and has been adapted, for tree and shrub species \citep{Yu2016,Klosterman2018,Montgomery2020}.
% TO 
% While researchers have refined thermal sum models for some well studied species, such as some [herbaceous? woody? what?] crops, to be more precise \citep{Kumudini2014,Abendroth2019}, the GDD model remains widely accepted, and has been far more widely adapted for wild tree and shrub species \citep{Yu2016,Klosterman2018,Montgomery2020}.
% CJC 10Feb: this was in response to a reviewer from AFM I believe so we can ditch now if you think that would be better! But I've updated the text according to your suggestions
...
%emw6Feb: Chilling and photoperiod could produce plasticity ... so tried to clarify below. 
...
%CJC 30Jan (cut due to R1 comment): Because urban sites strongly contribute to carbon sequestration \citep{Ziter2018}, these trends are important to understand to best predict plant development with warming.
...
%emw6Feb: Some folks would define provenance effects as the genetic component so we need to be more careful here -- see edits on first sentence....
%emw6Feb: Why are we cramming these into this paragraph:
% 3) all species will rely on photoperiod more as winters warm \citep{Way2015}, 
% 5) artificial light can counteract the daylength limitation \citep{Meng2022}. 
% into this paragraph? These two don't relate to provenance to me and the sentence is REALLY long. If you can help me understand what we need to adjust I can take another look at this... 
% CJC 10Feb These were suggested by the reviewers unfortunately, but we can cut Way 2015 I think. I've tried to make the Meng ref actually make sense here and I cut Way. Let me know what you think
