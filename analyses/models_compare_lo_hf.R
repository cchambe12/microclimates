...
hobo$spp <- sort(hobo$spp)

mod_station_lo_hf <- brm(gdd_lo ~ spp, data=station,
                      control=list(max_treedepth = 15,adapt_delta = 0.99),
                      iter=4000, warmup=2500)

save(mod_station_lo_hf, file="stan/mod_station_lo_hf.Rdata")

mod_hobo_lo_hf <- brm(gdd_lo_hobo ~ spp, data=hobo,
                   control=list(max_treedepth = 15,adapt_delta = 0.99),
                   iter=4000, warmup=2500)

save(mod_hobo_lo_hf, file="stan/mod_hobo_lo_hf.Rdata")
