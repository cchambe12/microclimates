...
if (use.hobo==FALSE & use.urban==TRUE & use.provenance==TRUE &
  ...
  #  5) Let's do a quick lmer model to test the fake data
  modtest <- lmer(gdd ~ urban + provenance + (urban + provenance|species), data=fakedata_ws_urb_prov) ## Quick look looks good!
}
...
if (use.hobo==TRUE & use.urban==TRUE & use.provenance==TRUE &
  ...
  #  5) Let's do a quick lmer model to test the fake data
  modtest <- lmer(gdd ~ urban + provenance + (urban + provenance|species), data=fakedata_hl_urb_prov) ## Quick look looks good!
}
