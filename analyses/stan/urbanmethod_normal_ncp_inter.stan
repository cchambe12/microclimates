...
model {
	...
	//target += normal_lpdf(to_vector(b_um) | 0, 40);
        ...
        target+= normal_lpdf(mu_a_sp | 300,40);
	      target+= normal_lpdf(sigma_a_sp | 0,40);
        
        target+= normal_lpdf(mu_b_urban_sp | 0,40);
	      target+= normal_lpdf(sigma_b_urban_sp | 0,20);
        
        target+= normal_lpdf(mu_b_method_sp | 0,40);
	      target+= normal_lpdf(sigma_b_method_sp | 0,20);
	      
	      target+= normal_lpdf(mu_b_um_sp | 0,40);
