...
# emw -- I think the place you need climate is for the hypothesis that hobos capture true climate better, not for "hobo loggers are less accurate measures of the same weather" so I will go forth assuming we're talking about the other hypothesis ...
...
fstarindsd <- 20 ## sigma_y in model output # emw -- again, this is not sigma_y most likely

#### This is where I test our hypothesis. This doesn't come out of the model directly
# emw -- I am confused, the other code has a similar line but ws_sd.... I don't think we need this type of thing in BOTH sets of R code, to me it only relates to one hypothesis
hobo_sd <- 20  ### adds variation to hobo logger estimates, rendering hobo loggers less accurate measures of same weather... I hope


### Now the climate data. No hobo logger variation because we want the weather to be the same but estimates to vary
dayz <- rep(1:daysperyr, nobs)
cc.arb <- 11  ## Arboretum clim data
sigma.arb <- 5 

cc.hf <- 9  ### Forest clim data
sigma.hf <- 5  
...
### I think this should just make the hobo logger less accurate...??? I hope.
  df.fstar$hyp_a <- ifelse(df.fstar$method=="hobo", 1, 0)  ## This won't be spit out of the model. If it's the hobo logger, make it a 1 if it's the weather station make it a 0
  df.fstar$gdd.noise <- df.fstar$gdd.noise + df.fstar$hyp_a * rep(rnorm(n=nspps, mean=0, sd=hobo_sd), each=ninds*nmethods) ### Now, I am just adding more sigma to the hobo logger fstar values, seen by sd=hobo_sd (which was 20)
