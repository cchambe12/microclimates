...
## load the model
load("stan/mod_hobo_dvr.Rdata")
load("stan/mod_station_dvr.Rdata")

gdddvrstation<-as.data.frame(tidy(mod_station_dvr, prob=0.5))
gdddvrhobo<-as.data.frame(tidy(mod_hobo_dvr, prob=0.5))
modoutput <- gdddvrstation #modelhere
...
muplot.station<-ggplot(modoutput, aes(x=`25%`, xend=`75%`, y=Jvar, yend=Jvar)) +
  ...
  coord_cartesian(xlim=c(-400, 500), clip = 'off') + ggtitle("A. Weather Station")
...
muplot.hobo<-ggplot(modoutput, aes(x=`25%`, xend=`75%`, y=Jvar, yend=Jvar)) +
  ...
  coord_cartesian(xlim=c(-200, 200), clip = 'off') + ggtitle("B. Hobo Loggers")
