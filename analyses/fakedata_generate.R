...
#  4) We need to make a random intercept model for each species
baseslope <- list(slope = mean(env.samples[,2]))
baseslope.mat <- matrix(unlist(baseslope), ncol = length(baseslope), nrow = nsp * ntot, byrow = TRUE)
## From Geoff's simulate-linear.R code in OSPREE: Which parameters are random?
random.slope <- grep(pattern = paste("slope", collapse = "|"), x = names(baseslope))
# Generate random intercepts (by species) 
for(i in 1:length(random.slope)){
  baseslope.mat[, i] <- sapply(1:nsp, FUN = function(X){
    rep(rnorm(n = 1, mean = baseslope[[random.slope[i]]], sd = 50), ntot)})}

#  5) Calculate response
response <- sapply(1:nrow(env.samples), FUN = function(x){
  rnorm(n = 1, mean = baseslope.mat[x, ], sd = 50)})
#  6) Make a dataframe of fake data
fakedata_ws <- cbind(data.frame(species = as.vector(sapply(1:nsp, FUN = function(x) rep(x, ntot))),
                                gdd = response, urban = rbinom(1000, 1, 0.5)))
