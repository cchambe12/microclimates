...
spnum <- length(unique(samptest$species))
pdf(file.path(figpath, paste("muplot", nameforfig, figpathmore, ".pdf", sep="")),
    width = width, height = height)
par(xpd=FALSE)
par(mar=c(5,7,3,10))
plot(x=NULL,y=NULL, xlim=xlim, yaxt='n', ylim=ylim,
     xlab="Model estimate change in day of budburst", ylab="", main=nameforfig)
axis(2, at=1:2, labels=rev(c("Intercept", "Sampling \nFrequency")), las=1)
...
legend(leg1, leg2, sort(unique(gsub("_", " ", samptest$species))), pch=my.pch[1:spnum],
