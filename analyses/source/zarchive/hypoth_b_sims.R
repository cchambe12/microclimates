...
df.fstar$hyp_b <- ifelse(df.fstar$method=="ws", 1, 0)  ## Similar to above but this won't be spit out of the model. If it's the weather station, make it a 1 if it's the hobo logger make it a 0
df.fstar$gdd.noise <- df.fstar$gdd.noise + df.fstar$hyp_b * rep(rnorm(n=nspps, mean=0, sd=ws_sd), each=ninds*nmethods) ### Now, instead of adding an effect, I am just adding more sigma to the weather station fstar values, seen by sd=ws_sd (which was 20)
