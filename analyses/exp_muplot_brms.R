...
muplotfx <- function(modelhere, nameforfig, width, height, ylim, xlim, leg1, leg2){
  ...
  axis(2, at=1:1, labels=rev(c("Provenance \nLatitude")),las=1)
  abline(v=0, lty=2, col="darkgrey")
  rownameshere <- c("provenance.lat")
  ppeffects <- c("provenance.lat") # or 1:4 here...
  for(i in 1:1){#i=1
    pos.y<-(1:1)[i]
  ...
  legend(leg1, leg2, sort(unique(gsub("_", " ", gdd$spp))), pch=my.pch[1:length(spnum)],
